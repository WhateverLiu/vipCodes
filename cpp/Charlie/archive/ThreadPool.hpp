...
struct ThreadPool
{
  ...
  void initialize(int && maxCore)
  {
    ...
    std::sort(tp, tp + maxCore - 1, [](
        const std::thread & x, const std::thread & y)->bool
    {
      return x.get_id() < y.get_id();
    });
    
    
    workerThreadIDs = new std::thread::id [maxCore - 1];
    for (int i = 0, iend = maxCore - 1; i < iend; ++i)
      workerThreadIDs[i] = tp[i].get_id();
  }
  ...
  ThreadPool() { haveFood = nullptr;  maxCore = 1; }
                ...
                [](std::size_t) { return false; })
  {
    ...
    // =========================================================================
    // Kick off secondary thread runs.
    // =========================================================================
    std::fill(this->haveFood, this->haveFood + this->maxCore, true);
    ...
    this->runJobs(0); 
    // =========================================================================
    // Will not go to the next line until the main thread finishes.
    // =========================================================================
    
    
    // =========================================================================
    // Check if all the secondary threads have finished.
    // =========================================================================
