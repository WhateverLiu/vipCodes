...
private:
  ...
  // ===========================================================================
  Vector xcp, vecc, vecs, vecy;
  Cauchy<Scalar> CCY;
  SubspaceMin<Scalar> SBM;
  LineSearch<Scalar> LSH;
  // ===========================================================================
...
public:
                      ...
                      const Vector& lb, const Vector& ub)
  {
    ...
    // std::cout << "f(x0) = " << fx << ", ||proj_grad|| = 
    // " << m_projgnorm << std::endl << std::endl;
    
    
    // Early exit if the initial x is already a minimizer
    if (m_projgnorm <= m_param.epsilon || 
        m_projgnorm <= m_param.epsilon_rel * x.norm())
    {
      return 1;
    }
    ...
    // =========================================================================
    // Cauchy<Scalar>::get_cauchy_point(
    CCY.get_cauchy_point(
      m_bfgs, x, m_grad, lb, ub, xcp, vecc, newact_set, fv_set);
    // =========================================================================
    ...
    /* Vector gcp(n);
     ...
     std::cout << "f(xcp) = " << fcp << ", ||proj_grad|| = " << 
       projgcpnorm << std::endl << std::endl; */
    ...
    for (;;)
    {
      ...
      //   f, m_param, m_xp, m_drt, step_max, step, fx, m_grad, dg, x);
      LSH.LineSearch(
        f, m_param, m_xp, m_drt, step_max, step, fx, m_grad, dg, x);
      // =======================================================================
      ...
      // =======================================================================
      // SubspaceMin<Scalar>::subspace_minimize(
      //   m_bfgs, x, xcp, m_grad, lb, ub,
      //   vecc, newact_set, fv_set, m_param.max_submin, m_drt);
      SBM.subspace_minimize(
        m_bfgs, x, xcp, m_grad, lb, ub,
        vecc, newact_set, fv_set, m_param.max_submin, m_drt);
      // =======================================================================
      ...
      /*Vector gsm(n);
       ...
       std::cout << "f(xsm) = " << fsm << ", ||proj_grad|| = " << 
         projgsmnorm << std::endl << std::endl;*/
  ...
  /// The projected gradient is defined as \f$P(x-g,l,u)-x\f$, where 
  ///  \f$P(v,l,u)\f$ stands for the projection of a vector \f$v\f$ onto 
  /// the box specified by the lower bound vector \f$l\f$ and
