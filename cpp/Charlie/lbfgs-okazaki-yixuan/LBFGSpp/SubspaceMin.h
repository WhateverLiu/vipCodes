...
private:
  ...
  //   return res;
  // }
  void subvec(const Vector& v, const IndexSet& ind, Vector &res)
  {
    const int nsub = ind.size();
    // Vector res(nsub);
    res.resize(nsub);
  ...
  Vector subvec(const Vector& v, const IndexSet& ind)
  {
    const int nsub = ind.size();
    Vector res(nsub);
    for (int i = 0; i < nsub; i++) res[i] = v[ind[i]];
  ...
  void subvec_assign(Vector& v, const IndexSet& ind, const Vector& rhs)
  {
    const int nsub = ind.size();
    for (int i = 0; i < nsub; i++)
      v[ind[i]] = rhs[i];
  ...
  bool in_bounds(const Vector& x, const Vector& lb, const Vector& ub)
  // ===========================================================================
  {
    const int n = x.size();
    for (int i = 0; i < n; i++)
  ...
  // static bool P_converged(const IndexSet& yP_set, const Vector& vecy, 
  //                         const Vector& vecl, const Vector& vecu)
  bool P_converged(const IndexSet& yP_set, const Vector& vecy, 
                   const Vector& vecl, const Vector& vecu)
  {
  ...
  bool L_converged(const IndexSet& yL_set, const Vector& lambda)  
  {
  ...
  bool U_converged(const IndexSet& yU_set, const Vector& mu)  
  {
  ...
  Matrix WF, WP;
  Vector vecc, vecl, vecu, vecy, yfallback, lambda, mu, rhs, lL, uU, tmp;
...
public:
  ...
  void subspace_minimize(
      BFGSMat<Scalar, true>& bfgs, const Vector& x0, const Vector& xcp, 
      const Vector& g, const Vector& lb, const Vector& ub, const Vector& Wd, 
      const IndexSet& newact_set, const IndexSet& fv_set, int maxit,
      Vector& drt)
  {
    ...
    vecl.resize(nfree); vecu.resize(nfree);
    // =========================================================================
    ...
    lambda.noalias() = Vector::Zero(nfree); 
    mu.noalias() = Vector::Zero(nfree);
    // =========================================================================
    ...
    // =========================================================================
    // IndexSet L_set, U_set, P_set, yL_set, yU_set, yP_set;
     L_set.resize(0);  U_set.resize(0);  P_set.resize(0); 
    yL_set.resize(0); yU_set.resize(0); yP_set.resize(0);
    // =========================================================================
    ...
    for (k = 0; k < maxit; k++)
    {
      ...
      if (nP > 0)
      {
        ...
        // Vector tmp(nP);
        rhs.resize(0); lL.resize(0); uU.resize(0); tmp.resize(nP);
        subvec(vecc, yP_set, rhs);
        subvec(vecl, yL_set, lL);
        subvec(vecu, yU_set, uU);
        // =====================================================================
      ...
      // =======================================================================
      // Vector Fy; // tmp occupied.
      Vector &Fy = tmp; Fy.resize(0);
      // =======================================================================
      ...
      if (nU > 0)
      {
        // =====================================================================
        // Vector negRes;
        Vector &negRes = rhs; negRes.resize(0);
        // =====================================================================
