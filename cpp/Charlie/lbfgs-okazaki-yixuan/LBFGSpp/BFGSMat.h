...
public:
  ...
  inline void Wb(int b, Vector &res)
  // ===========================================================================
  {
  ...
  inline void Wb(const IndexSet& b, Matrix &res)
  //============================================================================
  {
    const int nb = b.size();
    const int* bptr = b.data();
    // Matrix res(nb, 2 * m_ncorr);
    res.resize(nb, 2 * m_ncorr);
  ...
  inline void apply_Mv(const Vector& v, Vector& res)
  {
    ...
    vpadding.noalias() = Vector::Zero(2 * m_m);
    // =========================================================================
                         ...
                         bool test_zero = false)
  {
    ...
    // IndexSet P_reduced;
    // std::vector<Scalar> v_reduced;
    P_reduced.resize(0); v_reduced.resize(0);
    // =========================================================================
  ...
  // Returns false if the result is known to be zero
  Vector Mv;
  inline bool apply_PtWMv(const IndexSet& P_set, const Vector& v, Vector& res, const Scalar& scale)
  {
  ...
  inline bool apply_PtWMv(const Matrix& WP, const Vector& v, Vector& res, const Scalar& scale)
  {
  ...
  inline void compute_FtBAb(
      const Matrix& WF, const IndexSet& fv_set, const IndexSet& newact_set, 
      const Vector& Wd, const Vector& drt, Vector& res)
  {
    ...
    if (nact <= nfree)
    {
      ...
      // =======================================================================
      // Vector Ad(nfree);
      Vector &Ad = AdFd; Ad.resize(nfree);
      // =======================================================================
    ...
    else
    {
      ...
      // =======================================================================
      // Vector Fd(nfree);
      Vector &Fd = AdFd; Fd.resize(nfree);
      // =======================================================================
  ...
  //
  // v is [nP x 1]
  Matrix mid; Vector WPv;
  inline void solve_PtBP(const Matrix& WP, const Vector& v, Vector& res)
  {
    const int nP = WP.rows();
    res.resize(nP);
    if (m_ncorr < 1 || nP < 1)
    ...
    mid.resize(2 * m_ncorr, 2 * m_ncorr);
    //==========================================================================
  ...
  // Returns false if the result is known to be zero
  Vector WQtv, MWQtv;
  inline bool apply_PtBQv(const Matrix& WP, const IndexSet& Q_set, const Vector& v, Vector& res, bool test_zero = false)
  {
    const int nP = WP.rows();
  ...
  inline bool apply_PtBQv(const Matrix& WP, const Matrix& WQ, const Vector& v, Vector& res)
  {
    const int nP = WP.rows();
