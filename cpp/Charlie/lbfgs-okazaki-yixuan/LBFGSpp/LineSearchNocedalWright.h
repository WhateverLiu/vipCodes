...
public:
  ...
  void LineSearch(Foo& f, const LBFGSParam<Scalar>& param,
                  ...
                  Scalar& step, Scalar& fx, Vector& grad, Scalar& dg, Vector& x)
  {
    ...
    Scalar step_hi, fx_hi; // Warning: dg_hi is never used.
    // =====================================================================
    ...
    for (;;)
    {
      ...
      if (fx - fx_init > step * test_decr || (Scalar(0) < step_lo && fx >= fx_lo))
      {
        ...
        fx_hi = fx;
        // =============================================================
        // dg_hi = dg; // g++ warning: dg_hi never used.
        // =============================================================
        break;
      }
      ...
      fx_hi = fx_lo;
      // =================================================================
      // dg_hi = dg_lo; // g++ warning: dg_hi set but not used.
      // =================================================================
      step_lo = step;
      fx_lo = fx;
      dg_lo = dg;
    ...
    for (;;)
    {
      ...
      if (fx - fx_init > step * test_decr || fx >= fx_lo)
      {
        ...
        fx_hi = fx;
        // =============================================================
        // dg_hi = dg; // g++ warning: dg_hi set but not used.
        // =============================================================
      }
      else
      {
      ...
      {
        // throw std::runtime_error("the line search routine reached the maximum number of iterations");
        if (step_lo <= Scalar(0)) throw std::runtime_error(
